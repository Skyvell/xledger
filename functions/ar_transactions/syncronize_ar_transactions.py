...
from functions.ar_transactions.queries import (
    COLUMNS,
    GET_AR_TRANSACTION_DELTAS,
    GET_AR_TRANSACTIONS_AFTER_CURSOR,
    GET_AR_TRANSACTIONS_FROM_DBIDS
)
...
@bp.function_name("SyncronizeArTransactions")
@bp.schedule(schedule="0 0 * * * *", arg_name="myTimer", run_on_startup=True,
              use_monitor=False) 
def syncronize_ar_transactions(myTimer: func.TimerRequest) -> None:
    # Get credentials.
    ...
    delta_fetcher = DeltaFetcher(grapql_client, GET_AR_TRANSACTION_DELTAS)
    item_fetcher = ItemFetcher(grapql_client, GET_AR_TRANSACTIONS_FROM_DBIDS, GET_AR_TRANSACTIONS_AFTER_CURSOR)
