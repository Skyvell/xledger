...
from functions.suppliers.queries import (
    COLUMNS,
    GET_SUPPLIERS_DELTAS,
    GET_SUPPLIERS_FROM_DBIDS,
    GET_SUPPLIERS_AFTER_CURSOR
)
...
@bp.function_name("SyncronizeSuppliers")
@bp.schedule(schedule="0 0 * * * *", arg_name="myTimer", run_on_startup=True,
              use_monitor=False) 
def syncronize_suppliers(myTimer: func.TimerRequest) -> None:
    # Get credentials.
    ...
    delta_fetcher = DeltaFetcher(grapql_client, GET_SUPPLIERS_DELTAS)
    item_fetcher = ItemFetcher(grapql_client, GET_SUPPLIERS_FROM_DBIDS, GET_SUPPLIERS_AFTER_CURSOR)
