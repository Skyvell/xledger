...
from functions.transactions.queries import (
    COLUMNS,
    GET_TRANSACTION_DELTAS,
    GET_TRANSACTIONS_AFTER_CURSOR,
    GET_TRANSACTIONS_FROM_DBIDS
)
...
@bp.function_name("SyncronizeTransactions")
@bp.schedule(schedule="0 0 * * * *", arg_name="myTimer", run_on_startup=True,
              use_monitor=False) 
def syncronize_transactions(myTimer: func.TimerRequest) -> None:
    # Get credentials.
    ...
    delta_fetcher = DeltaFetcher(grapql_client, GET_TRANSACTION_DELTAS)
    item_fetcher = ItemFetcher(grapql_client, GET_TRANSACTIONS_FROM_DBIDS, GET_TRANSACTIONS_AFTER_CURSOR)
